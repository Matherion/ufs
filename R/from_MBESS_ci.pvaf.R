from_MBESS_ci.pvaf <- function (F.value = NULL, df.1 = NULL, df.2 = NULL, N = NULL,
                                ...
                                ...)
{
  ...
  Lims <- from_MBESS_conf.limits.ncf(F.value = F.value, conf.level = NULL,
                          df.1 = df.1, df.2 = df.2, alpha.lower = alpha.lower,
                          alpha.upper = alpha.upper, ...)
  if (is.na(Lims$Lower.Limit)) {
    Lims$Lower.Limit <- 0
  }
  if (is.na(Lims$Prob.Less.Lower)) {
    Lims$Prob.Less.Lower <- 0
  }
